    ...
    <div className="relative w-fit h-10">
      {!isSearching ? (
        <button 
          ...
          className="w-full bg-[#FCD24C] hover:bg-[#FF7300] text-black font-bold px-8 py-2 rounded-full 
                     text-sm transition-all shadow-[5px_5px_0_0_rgb(133_82_37_/_0.4)] border-black border-[2px] active:scale-95 whitespace-nowrap"
        >
          {selectedCafe || "Choose a cafe"}
        </button>
      ) : (
        <div className="flex flex-col w-[400px] absolute top-0 left-0 z-50">
          <input 
            autoFocus
            type="text"
            ...
            className="w-full bg-white border-2 border-black rounded-xl px-4 py-2 font-bold 
                       focus:outline-none shadow-[5px_5px_0_0_rgb(133_82_37_/_0.2)] text-sm"
          />
          
          {filteredCafes.length > 0 && (
            <div className="mt-2 bg-white border-2 border-black rounded-xl overflow-hidden 
                            shadow-[5px_5px_0_0_rgb(133_82_37_/_0.2)] max-h-[200px] overflow-y-auto">
              {filteredCafes.map((cafe) => (
                <button 
                  key={cafe}
                  onMouseDown={() => handleSelect(cafe)} 
                  className="w-full text-left px-4 py-3 hover:bg-[#FBF3DE] font-bold 
                             border-b border-black/5 last:border-none text-sm text-black"
                >
                  {cafe}
                </button>
              ))}
